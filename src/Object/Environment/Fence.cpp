...
Fence::Fence(int handle, Vector3 create_pos, Vector3 create_rot)
{
	...
	*object_model_ = MV1DuplicateModel(handle);
	...
	size_pos[2].set(+collision_size_.x, collision_size_.y, +collision_size_.z);		// 右下
	size_pos[3].set(-collision_size_.x, collision_size_.y, +collision_size_.z);		// 左下
...
void Fence::Render()
{
	...
	MV1SetRotationXYZ(*object_model_, VGet(object_rotate_.x, object_rotate_.y, object_rotate_.z));
